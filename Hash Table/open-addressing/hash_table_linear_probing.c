...
void insertOpenAddressing(struct HashTable* ht, int key) {
    int index = hash(key);
    int startIndex = index;  // To detect when we've looped through the table

    while (ht->status[index] == 1) { // Slot is occupied
...
int searchOpenAddressing(struct HashTable* ht, int key) {
    int index = hash(key);
    while (ht->status[index] != 0) {  // Continue until empty slot
...
int main() {
    ...
    insertOpenAddressing(&ht, 45);  // Try to insert a fourth element
